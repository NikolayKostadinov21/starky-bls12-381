...
use crate::{
    constants::{BLS_N_LIMBS, LIMB_BITS},
    modular::{
        modular::AUX_COEFF_ABS_MAX,
        pol_utils::{
            pol_add_assign, pol_add_assign_ext_circuit, pol_adjoin_root,
            pol_adjoin_root_ext_circuit, pol_mul_wide2, pol_mul_wide2_ext_circuit,
            pol_remove_root_2exp, pol_sub_assign, pol_sub_assign_ext_circuit,
        },
    },
    utils::utils::columns_to_bigint,
};
...
pub fn generate_modular_zero<F: PrimeField64>(
    ...
    let quot_limbs = bigint_to_columns::<{ BLS_N_LIMBS + 1 }>(&quot);
    let quot_abs_limbs = bigint_to_columns::<{ BLS_N_LIMBS + 1 }>(&quot.abs());
    // constr_poly = zero_pol  - s(x)*m(x).
    let mut constr_poly = [0i64; 2 * BLS_N_LIMBS];
    constr_poly[..2 * BLS_N_LIMBS - 1].copy_from_slice(&zero_pol);
...
pub fn eval_modular_zero<P: PackedField>(
    ...
    // constr_poly = q(x) * m(x)
    let mut constr_poly: [_; 2 * BLS_N_LIMBS] = pol_mul_wide2(quot.try_into().unwrap(), modulus);
...
pub fn eval_modular_zero_circuit<F: RichField + Extendable<D>, const D: usize>(
    ...
    // constr_poly = q(x) * m(x)
    let mut constr_poly: [_; 2 * BLS_N_LIMBS] =
...
pub fn write_modulus_aux_zero<F: Copy>(lv: &mut [F], aux: &ModulusAuxZero<F>, cur_col: &mut usize) {
    lv[*cur_col..*cur_col + BLS_N_LIMBS + 1].copy_from_slice(&aux.quot_abs);
    lv[*cur_col + BLS_N_LIMBS + 1..*cur_col + 3 * BLS_N_LIMBS].copy_from_slice(&aux.aux_input_lo);
    lv[*cur_col + 3 * BLS_N_LIMBS..*cur_col + 5 * BLS_N_LIMBS - 1]
        .copy_from_slice(&aux.aux_input_hi);
    *cur_col += 5 * BLS_N_LIMBS - 1;
}
...
pub fn read_modulus_aux_zero<F: Copy + fmt::Debug>(
    ...
    let quot_abs = lv[*cur_col..*cur_col + BLS_N_LIMBS + 1].to_vec();
    let aux_input_lo = lv[*cur_col + BLS_N_LIMBS + 1..*cur_col + 3 * BLS_N_LIMBS].to_vec();
    let aux_input_hi = lv[*cur_col + 3 * BLS_N_LIMBS..*cur_col + 5 * BLS_N_LIMBS - 1].to_vec();
