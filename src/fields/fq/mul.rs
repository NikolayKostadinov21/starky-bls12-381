...
use crate::modular::modular::{
    ...
    eval_modular_op, eval_modular_op_circuit, generate_modular_op, read_modulus_aux, read_u256,
    write_modulus_aux, write_u256, ModulusAux,
};
use crate::modular::pol_utils::{pol_mul_wide, pol_mul_wide_bls, pol_mul_wide_ext_circuit};
use crate::utils::utils::positive_column_to_i64;

pub struct FqOutput<F> {
    pub output: [F; BLS_N_LIMBS],
...
pub fn generate_fq_mul<F: PrimeField64>(x: [F; BLS_N_LIMBS], y: [F; BLS_N_LIMBS]) -> FqOutput<F> {
    let modulus = bn254_base_modulus_bigint();
    let x_i64 = positive_column_to_i64(x);
    let y_i64 = positive_column_to_i64(y);
    let pol_input = pol_mul_wide_bls(x_i64, y_i64);
